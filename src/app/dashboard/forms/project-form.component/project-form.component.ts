...
import { PaginatedResponse } from '../../../services/pagination/pagination';
...
export class ProjectFormComponent implements OnInit {
	...
	public readonly availableTeams = computed<Team[]>(() => {
		...
		const userTeams = this.teamService.myTeams();
		if (Array.isArray(userTeams)) {
			return userTeams;
		}
		return (userTeams as PaginatedResponse<Team> | null)?.items ?? [];
	});
